...
## Follow-up — 2026-06-19T17:42:10+03:00

# Teamwork Project Prompt

> Goal: Replace hardcoded demo data in the D&D Companion App dashboards with live Supabase data, enabling GMs to manage actual characters and ensuring players see their real character stats.
...
The `CampaignContext` must be updated to fetch full character relations (e.g., joining `race_definitions` and `subclass_definitions` with `class_categories`). The `DashboardPage` and `GMDashboard` must be completely stripped of hardcoded demo data (like "14,500 XP", "Half-Elf", "Paladin", and "Kael the Bold") and instead use the active character's actual data from the database. This includes fixing the bug where newly created characters (like "BRT" the Brain Eater / Psycho Mage) incorrectly display as "Half-Elf Paladin".

### R2. GM Character Management
The GM Dashboard must allow the GM to view the real characters in the campaign within the Party and XP tabs. Furthermore, the GM must have the ability to manage these settings (create characters, set XP, Level, Race, Class, and Subclass) and assign them to users. Any XP distribution or character updates made by the GM must be saved to the Supabase database.

## Acceptance Criteria

### Live Data Integration
- [ ] `CampaignContext` correctly fetches characters with their associated race and subclass details.
- [ ] `DashboardPage` dynamically displays the logged-in user's active character name, level, XP, race, and class.
- [ ] Creating a new character with specific traits (e.g., Brain Eater Psycho Mage) correctly displays those traits on the dashboard, completely eliminating the "Half-Elf Paladin" hardcoded fallback.

### GM Dashboard Functionality
- [ ] The GM Dashboard's Party and XP tabs display the actual characters retrieved from Supabase, not `INITIAL_PLAYERS`.
- [ ] The GM can create a new character, specifying Name, Race, Class, Subclass, Level, and XP.
- [ ] The GM can distribute XP to characters, and the changes persist in the `characters` table in Supabase.
